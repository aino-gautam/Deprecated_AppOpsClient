package in.appops.client.common.snippet;

public class SnippetConstant {

	public static final String  IMAGEUPLOAD = "ImageUpload";
	public static final String  SERVICEICONSNIPPET = "ServiceIconSnippet";
	public static final String  SERVICEENTITYHOMESNIPPET = "ServiceEntityHomeSnippet";
	public static final String  MEDIASERVICEHOME = "mediaServiceHome";
	public static final String  CALENDARSERVICEHOME = "calendarServiceHome";
	public static final String  CONTACTSERVICEHOME = "contactServiceHome";
	public static final String  POSTSERVICEHOME = "postServiceHome";
	public static final String  LISTSNIPPET = "listSnippet";
	public static final String  REMINDERSNIPPET = "reminderSnippet";
	public static final String  POSTVIEWSNIPPET = "postViewSnippet";
	public static final String  HOMESNIPPET = "homeSnippet";
}
