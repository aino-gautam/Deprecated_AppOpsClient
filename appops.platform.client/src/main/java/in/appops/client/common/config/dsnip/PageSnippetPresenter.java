...
public class PageSnippetPresenter extends HTMLSnippetPresenter {
	...
	private void createAddSnippet(String transformToSnippet, String transformFromSnippet, String snippetInstance) {
		try {
			...
			((PageSnippetView) view).getElementMap().put(transformFromSnippet, snippetTo);
		} catch (Exception e) {
			e.printStackTrace();
		}		
	}
			...
			*/
			...
			while(!preparedConfigMap.isEmpty()) {
				for(Map.Entry<String, Object> configToUpdateEntry : preparedConfigMap.entrySet()) {
					...
					Object propertyValue = configToUpdateEntry.getValue(); // <value>
					...
					if((snippetToUpdate == null && (snippetToUpdate = (HTMLSnippetPresenter) ((PageSnippetView) view).getElementMap().get(snippetInstance)) != null ) || 
						...
						snippetToUpdate.getModel().updateConfiguration(propertyToUpdate, (Serializable)propertyValue);
